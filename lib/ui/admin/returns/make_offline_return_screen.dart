...
class MakeOfflineReturnScreen extends StatefulWidget {
  ...
  @override
  State<MakeOfflineReturnScreen> createState() =>
      _MakeOfflineReturnScreenState();
}

class _MakeOfflineReturnScreenState extends State<MakeOfflineReturnScreen> {
  int _step = 0;
              ...
              child: Row(children: [
                ...
                Container(
                  ...
                  child: Text('${_step + 1}/3',
                      style: manrope(12, FontWeight.w700, color: Colors.white)),
                ),
              ]),
            ),
          ),
          ...
          child: _step == 0
              ...
              : _step == 1
                  ? _StepItems(
                      itemMap: _itemMap,
                      reason: _reason,
                      onItemToggled: (key, val) =>
                          setState(() => _itemMap[key] = val),
...
class _StepFind extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      ...
      if (loading)
        const Expanded(
            child: Center(child: CircularProgressIndicator(color: cGreen)))
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (_, i) =>
                _SaleResultCard(sale: sales[i], onTap: () => onSelect(sales[i])),
          ),
        ),
    ]);
  }
...
class _SaleResultCard extends StatelessWidget {
  final SaleModel sale;
  final VoidCallback onTap;
  const _SaleResultCard({required this.sale, required this.onTap});
...
class _StepRefund extends StatelessWidget {
