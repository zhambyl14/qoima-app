...
import '../courier/courier_deliveries_screen.dart';
...
class _AdminHomeScreenState extends State<AdminHomeScreen> {
  ...
  late Future<int> _pairsFuture;

  @override
  void initState() {
    super.initState();
    _pairsFuture = _countTotalPairs();
  }

  Future<int> _countTotalPairs() async {
    try {
      final (batchesMap, _) = await _service.getAllBatchesGrouped();
      int total = 0;
      for (final batches in batchesMap.values) {
        for (final batch in batches) {
          // totalAvailable correctly subtracts reserved_sizes (online reservations)
  ...
  Future<void> _onRefresh() async {
    ...
    if (mounted) {
      setState(() => _pairsFuture = _countTotalPairs());
    }
  }
          ...
          child: RefreshIndicator(
            ...
            child: ListView(
              ...
              children: [
                ...
                Row(children: [
                  ...
                  Expanded(
                    child: StreamBuilder<List<SaleModel>>(
                      stream: _service.watchSalesForMonth(DateTime.now()),
                      builder: (_, s) {
                        final total = (s.data ?? [])
                  ...
                  // Pairs on warehouses card — refreshes with pull-to-refresh
                  Expanded(
                ...
                // Online orders block
                StreamBuilder<int>(
                  stream: _service.watchActiveOnlineCount(),
                  builder: (_, s) {
                ...
                StreamBuilder<List<CourierDeliveryModel>>(
                  ...
                  builder: (_, snap) {
                    ...
                    return _QMenuItem(
                      ...
                      sub: newCount == 0
                          ? 'Нет новых доставок'
                          : '$newCount новых ждут курьера',
                      badge: newCount > 0 ? '$newCount' : null,
                      onTap: () => Navigator.push(
                          context,
                          MaterialPageRoute(
                              builder: (_) => const CourierDeliveriesScreen())),
                    );
                  },
                ),
              ],
