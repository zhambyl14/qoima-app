...
class _ClientFiltersSheetState extends State<ClientFiltersSheet> {
  ...
  late List<String> _allCategoryKeys; // тип товара v2, в порядке kCategories
  ...
  void initState() {
    ...
    for (final pair in widget.pairs) {
      ...
      presentKeys.add(p.effectiveCategoryKey);
    ...
    _allCategoryKeys =
        kCategories.map((c) => c.key).where(presentKeys.contains).toList();
  }
  ...
  int get _resultCount {
    return widget.pairs.where((pair) {
      ...
      if (_f.categoryKeys.isNotEmpty &&
          !_f.categoryKeys.contains(p.effectiveCategoryKey)) {
        return false;
      }
              ...
              children: [
                ...
                if (_allCategoryKeys.length > 1) ...[
                  ...
                  Wrap(
                      spacing: 8,
                      runSpacing: 8,
                      children: _allCategoryKeys.map((k) {
                        final cat = categoryByKey(k);
                        final sel = _f.categoryKeys.contains(k);
                        final tone = toneFor(cat.tone);
                        return GestureDetector(
                          onTap: () => setState(() {
                            final s = Set<String>.from(_f.categoryKeys);
                            sel ? s.remove(k) : s.add(k);
                            _f = _f.copyWith(categoryKeys: s);
                          }),
                          child: AnimatedContainer(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: sel ? cGreen : cLine),
                            ),
                            child: Row(mainAxisSize: MainAxisSize.min, children: [
                              CategoryIcon(
                                  kind: cat.iconKey,
                                  size: 14,
                                  color: sel ? Colors.white : tone.deep),
